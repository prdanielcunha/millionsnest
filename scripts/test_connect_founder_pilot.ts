...
const normalExperience = resolveHubAppExperience({ app: connect, organization, isGlobalAdmin: false });
...
resolveHubAppCatalog(ECOSYSTEM_APPS, { organization: normalCatalogOrganization, isGlobalAdmin: false });
assert.deepEqual(normalCatalogOrganization.enabledApps, [], 'normal users must not receive an in-memory Connect enablement projection');

const adminExperience = resolveHubAppExperience({ app: connect, organization, isGlobalAdmin: true });
...
const adminCatalog = resolveHubAppCatalog(ECOSYSTEM_APPS, { organization: adminCatalogOrganization, isGlobalAdmin: true });
...
assert.deepEqual(adminCatalogOrganization.enabledApps, ['musicscale', 'connect'], 'admin founder pilot projects Connect into the current in-memory Hub launch session');
assert.equal(adminCatalogOrganization.enabledApps.filter(appId => appId === 'connect').length, 1, 'projection must never duplicate Connect');
resolveHubAppCatalog(ECOSYSTEM_APPS, { organization: adminCatalogOrganization, isGlobalAdmin: true });
assert.equal(adminCatalogOrganization.enabledApps.filter(appId => appId === 'connect').length, 1, 'repeated catalog resolution keeps the projection idempotent');
