...
assert.equal(hosting.target, "hub", "MillionsNest must deploy only to its dedicated Hosting target");
...
assert.equal(rewrites[0]?.run?.serviceId, "millionsnest-api", "API must route to the canonical Cloud Run service");
...
assert.match(dockerfile, /FIREBASE_PROJECT_ID=millionsnest/, "Cloud Run must use the canonical Firebase project through ADC");
assert.doesNotMatch(dockerfile, /STRIPE_SECRET_KEY=|STRIPE_WEBHOOK_SECRET=|FIREBASE_PRIVATE_KEY=|FIREBASE_SERVICE_ACCOUNT_BASE64=/, "No production secret may be baked into the image");

const server = fs.readFileSync("server.ts", "utf8");
assert.match(server, /process\.env\.PORT\s*\|\|\s*3000/, "Server must honor Cloud Run's PORT environment variable");
assert.match(server, /app\.post\('\/api\/stripe\/webhook', express\.raw\(/, "Stripe webhook raw-body contract must remain present");
assert.match(server, /if \(!process\.env\.VERCEL\)/, "Vercel handler compatibility must remain during rollback period");
...
console.log("MillionsNest Firebase Hosting + Cloud Run migration contract: OK");
